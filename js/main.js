...
const API = "http://localhost:8001/info";
...
paginPrevBtn.addEventListener("click", () => {
  ...
  currentPage--;
  currentPageAdmin--;
  localStorage.getItem("isAdmin") ? readCardAdmin() : readCard();
});
...
async function passwordValue() {
  let res = await fetch("http://localhost:8001/admin");
  let data = await res.json();
  console.log(data);
...
async function readCardAdmin(letter = "") {
  let res =
    filterValue !== "Все"
      ? await fetch(
          `${API}?q=${letter}&_page=${currentPageAdmin}&_limit=3&marka=${filterValue}`
        )
      : await fetch(`${API}?q=${letter}&_page=${currentPageAdmin}&_limit=3`);
  let data = await res.json();
  cardsContainer.innerHTML = "";
  if (JSON.parse(localStorage.getItem("isAdmin")) === true) {
    ...
    <p>${elem.price}</p>
    <p>${elem.year}</p>
    <button onclick="deleteInfo(${elem.id})">delete</button>
    <button onclick="showEditModal(${elem.id})">edit</button>
    </div>
    `;
    });
    countPages();
